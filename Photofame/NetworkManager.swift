...
class NetworkManager: NSObject {
    ...
    class func uploadImageData(params: [String: Any], completion: @escaping ([String:Any]?) -> ()) {
        
        if let jsonData = try? JSONSerialization.data(withJSONObject: params, options: []) {
            
            let url = NSURL(string: "http://api.photofame.online/uploadMedia")!
            let request = NSMutableURLRequest(url: url as URL)
            request.httpMethod = "POST"
            request.addValue("application/json", forHTTPHeaderField: "Content-Type")
            request.addValue("application/json", forHTTPHeaderField: "Accept")
                ...
                do {
                    ...
                    if let parseJSON = json {
                        ...
                        if let code = parseJSON["code"] as? Int {
                            ...
                            if code == 200 {
                                completion(parseJSON as? [String : Any])
                            }
                        }
                    }
                    completion(nil)
